...
fun SearchRoute(navController: NavHostController, modifier: Modifier) {
    SearchScreen(
        modifier = modifier,
        navigateToResult = { navController.navigate(SearchResult(it)) },
...
fun SearchScreen(
    ...
    Column(
        ...
        Row(modifier = Modifier.padding(horizontal = 16.dp), verticalAlignment = CenterVertically) {
            SearchText(viewModel, navigateToResult)
        ...
        Row(
            ...
            if (keywords is StateSuccess && keywords.data?.isNotEmpty() == true) {
                Text(
                    ...
                    style = typography.titleSmall.copy(fontWeight = W600),
                    color = colorScheme.primary,
                    maxLines = 1,
                    modifier = Modifier
                        .padding(end = 16.dp)
        ...
        if (keywords is StateSuccess) {
            KeywordsStateSuccess(
                keywords = keywords.data.orEmpty(),
                onDeleteClick = { deleteKeyword(it) },
    ...
    BasicTextField(
        ...
        keyboardActions = KeyboardActions(onDone = {
            ...
            if (searchText.isNotEmpty() && searchText.isNotBlank()) {
                viewModel.insertKeyword(searchText)
            }
...
private fun KeywordsStateSuccess(
    ...
    onDeleteClick: (SearchKeywordDto) -> Unit,
    navigateToResult: (String) -> Unit
) {
    val interactionSource = remember { MutableInteractionSource() }
        ...
        items(keywords.size) {
            Column(
                modifier = Modifier.clickable(
                    interactionSource = interactionSource,
                    indication = null
                ) { navigateToResult(keywords[it].keyword) }
            ) {
